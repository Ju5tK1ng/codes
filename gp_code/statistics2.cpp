...
string DATA_PATH = "D:\\codes\\gp_code\\dataDividend.csv";
// string DATA_PATH = "S:\\codes\\gp\\dataEtf.csv";
string line;
vector<string> dataList;
vector<DayData> dayDataList;
vector<string> START_DATE = {"2019-12-20"};
...
int main()
{
    ...
    for (int i = 1; i < dataList.size(); i++)
    {
        ...
        if (start)
        {
            double percent = floor((dayData.high - lastClose) / lastClose * 100 / interval) * interval;
            percent2CntMap[percent] += 1;
            percent = ceil((dayData.low - lastClose) / lastClose * 100 / interval) * interval;
            percent2CntMap[percent] += 1;
            cnt += 2;
        }
        lastClose = dayData.close;
    }
    string totalPath = "D:\\gp\\data\\statistics2" + START_DATE[0] + '_' + float2Str(interval) + ".csv";
